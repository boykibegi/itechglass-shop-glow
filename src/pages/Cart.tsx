        ...
        <div className="container">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-4">
              {items.map((item) => (
                <div key={`${item.productId}-${item.selectedModel}`} className="flex gap-4 p-4 bg-card rounded-lg border border-border">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground">{t('cart.model')}: {item.selectedModel}</p>
                    <p className="text-lg font-semibold mt-1">TSh {item.price.toLocaleString()}</p>
                  </div>

                  <div className="flex flex-col items-end justify-between">
                    <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-destructive" onClick={() => removeItem(item.productId, item.selectedModel)}>
                      <Trash2 className="h-4 w-4" />
                    </Button>
                    
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => updateQuantity(item.productId, item.selectedModel, item.quantity - 1)}>
